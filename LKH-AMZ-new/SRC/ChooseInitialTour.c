 ...
 *  (C) Otherwise, if possible, choose NextN such that (N,NextN) is a
 *      candidate edge.
 *  (D) Otherwise, choose NextN at random among those nodes not already
 *      chosen.
 *
...
Start:
    ...
    while (N->Suc != FirstNode) {
        ...
        if (Alternatives == 0) {
            /* Case D (actually not really a random choice) */
            NextN = N->Suc;
